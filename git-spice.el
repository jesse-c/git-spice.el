;;; git-spice.el --- Git-spice integration for Magit -*- lexical-binding: t -*-
...
;; This package provides Emacs integration for git-spice, a tool for
;; stacking Git branches (https://github.com/abhinav/git-spice).
;;
;; Features:
;; - Transient menu interface for git-spice commands
;; - Magit status section showing your branch stacks
;; - Automatic refresh after git-spice operations
;; - Success/failure reporting for operations
;;
;; Usage:
;;
;;   M-x git-spice-menu                ; Open the main transient menu
;;   M-x git-spice-setup-magit-section ; Enable Magit integration
;;
;; The transient menu provides access to:
;; - Repository operations (sync with --restack)
;; - Branch management (create, checkout, track, delete, restack)
;; - Stack operations (submit, restack, edit)
;; - Upstack/downstack operations
;; - Commit operations (create, amend, split, fixup)
;; - Navigation between branches in the stack
;;
;; Requirements:
;; - git-spice must be installed and available in PATH
...
(defgroup git-spice nil
  "Git-spice integration for Emacs."
  :group 'vc
      ...
      (let ((child-prefix (concat prefix (if is-last "   " "┃")))
        ...
        (dotimes (i child-count)
          (let* ((child-ref (nth i ups))
            ...
            (when child-branch
              (git-spice-insert-branch-tree child-branch branches-by-name
                                            child-prefix is-last-child))))))))

(defun git-spice-magit-insert-section ()
...
;;;###autoload
(defun git-spice-setup-magit-section ()
  "Enable git-spice section in Magit status buffer."
  (interactive)
  (add-hook 'magit-status-sections-hook 'git-spice-magit-insert-section t))

(defun git-spice-teardown-magit-section ()
  "Disable git-spice section in Magit status buffer."
  (interactive)
  (remove-hook 'magit-status-sections-hook 'git-spice-magit-insert-section t))
...
(defun git-spice-branch-create (name)
  "Create a new branch with NAME using transient arguments."
  (interactive "sBranch name: ")
  (apply #'git-spice-run "branch" "create" (append (git-spice-arguments 'git-spice-branch-create-menu) (list name))))
...
(transient-define-prefix git-spice-menu ()
  ...
  [["Repository"
   ...
   ["Branch"
    ("b" "Create›" git-spice-branch-create-menu)
